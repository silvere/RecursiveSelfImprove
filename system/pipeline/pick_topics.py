...
def render_entry(e: dict, bank: Path) -> str:
    ...
    # 适配判断为什么写成必答题而不是自动打分：L-033 拿库里现成的 dimensions.scale 对
    # 08-14~08-17 的 12 条候选回跑过，选中组与淘汰组取值分布几乎重合，零判别力；而人工
    # 复盘那 12 条得出的唯一真实分界是"这个问题的主语是不是人的处境"——它是语义判断，
    # 现有结构化字段无一能代理。所以脚本不打分，只把这一问顶到会话眼前逼它明写答案。
    lines += [
        "",
        "**适配判断（必答，先答这条再谈角度）**：这个问题的主语是**人的处境**，"
        "还是**某个系统/机构应当如何**？后者一律淘汰，哪怕 importance 很高。",
