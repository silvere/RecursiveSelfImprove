...
- [ ] **今日篇同步草稿箱（唯一未闭环项，堵在人）**｜文章已成稿、门禁通过、配图 CI 成功、代码已推（AIWriter `a329527`），**只差最后一步同步失败**：微信 `errcode=40164`，本机公网出口 IP `104.168.14.206` 不在公众平台 IP 白名单（断网 3 天恢复后 IP 变更）｜**这一步系统做不了**——改白名单要登录微信公众平台后台，属人独占动作，已按策略 9 闸一判定并用人话通知（`om_x100b674d447a5ca0c2b397e9f644da8`）｜**默认值（策略 9 闸二，第一次提问就写死）**：若 2026-08-23 前人未加白名单，下一场起停止追问，改为在每篇产出记录里如实标注"已成稿未同步"，并把"IP 白名单会随运营商变动"作为长期堵点写入简报请人裁决是否换方案 ｜**下一场动作**：先跑 `gh run rerun 32429558883 -R silvere/aiwriter` 重试同步，成功即拿 media_id 回填本日产出记录

- [ ] **T-021 题图指纹判定**（承接策略 10）｜按新落地的**策略 10 扰动实测**执行：上线前除证明"正常出图时放行"（策略 8 正常态取值）外，**必须实测"把当篇封面换成上一篇那张，判定要拒"**；两组证据缺一不得上线 ｜08-17 已备好零成本实现路径：git blob sha 天然是内容指纹
