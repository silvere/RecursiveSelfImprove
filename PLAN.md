...
- [ ] T-027 **复验模糊量级词机检闸门（⑤a）真的把 R-01 打到 0**（承接 T-026 判定未通过，见 L-041/L-044）｜下一篇走 Stage 5 时必须先跑 `python3 /Users/jingweisun/Code/AIWriter2/tools/vague_quantifier_check.py <文章目录>/05-draft.md`，命中即改完再进七刀 ｜**验收：下一篇 `06-review.md` 的 R-01 计数为 0，且 06-review 里注明机检闸门在草稿阶段拦下了几处**（若机检 0 处而审稿刀仍抓到 R-01，说明正则漏型，把漏掉的句式补进 PATTERNS 而不是回去改 prompt）｜**另有一处已知欠账**：08-17 那篇 `final.md` 至今留着一处漏网的"我读了很多遍"（已同步进草稿箱，L-044 证据③），**不追改已同步文章**（改了 media_id 不变、读者也看不到差异，属于给自己看的补作业），仅在此登记
...
（**本段清空于 2026-08-21 pm**：本场无遗留在途任务，日拱一卒已闭环。

**给 08-22 s1 的排序建议**：①**文章仍是唯一第一优先级**——C1 今天是 1/14，剩余容错只有 5 天（L-045 算术），任何基础设施任务都排在它后面；②Stage 5 写完 `05-draft.md` **必须先跑 ⑤a 机检闸门**再进七刀，这是 T-027 的验收动作，跑完把命中数写进 `06-review.md`；③配图 CI 成功后、同步草稿箱之前跑 `run_daily.sh cover-check posts/<日期>/<slug>`（退出码 4 = 题图重复）；④T-024（漏行）仍未修，线索是"成功场也会漏、不稳定复现"，去查成功路径而非崩溃路径——但**不要插队到文章前面**。）
