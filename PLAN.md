...
- [ ] T-021 **题图质量进入验收视野**（W-006，**最高优先级**）｜连续四篇共用同一张库存封面，系统三天无感，最后由**仓库外**的 AIWriter Bot（commit `6608f14`）修掉；而 LEDGER 每天记的 `uploaded_image_count: 1` **恒为 1**，与"是不是同一张图"无关 ｜验收：产出记录新增题图指纹字段，`run_daily.sh check` 阶段比对当篇封面 sha1 与最近 7 篇是否重复，重复即拒；并跑一次存量回跑证明它对本周四篇给出正确判定（**按策略 8：上线前先算正常态取值**）
...
- [ ] T-026 **验证第 ⑤ 条事实纪律真的堵住了上游**（本场 L-034 新增，明早成稿后即可判）｜下一篇 `06-review.md` 里"修辞冒充统计"类必修项应为 **0**；仍非 0 说明堵漏失败，要改的是写法不是重申规则 ｜验收：在下一篇的 06-review.md 规则库复查清单里对 R-01 给出计数结论，并回填 LEDGER L-034
...
（**08-18 全天零产出**：s1/s2/s3 三场因人的 Claude 登录过期全部空跑 8 秒内退出，无一场领到任务；晚场已拆掉这堵墙（L-036 凭证预检 + 可执行告警文案）。下一场（08-19 s1）**第一优先级是补上文章**——断档一天，候选文件 `workspace/topics/2026-08-19.md` 已生成，全程走 `run_daily.sh ship`；成稿后顺手验掉 T-026。若有余力再推 **T-021 题图**——它仍是 W 系列里唯一直接关于内容质量的一条，且 08-17 s2 已把最贵的一步铺好：git blob sha 就是现成的题图指纹。）
