...
- [ ] T-027 **复验模糊量级词机检闸门（⑤a）真的把 R-01 打到 0**（承接 T-026 判定未通过，见 L-041/L-044）｜下一篇走 Stage 5 时必须先跑 `python3 /Users/jingweisun/Code/AIWriter2/tools/vague_quantifier_check.py <文章目录>/05-draft.md`，命中即改完再进七刀 ｜**验收（2026-08-22 s1 改为机器可判，不再靠会话手写）**：`python3 /Users/jingweisun/Code/AIWriter2/tools/vague_quantifier_check.py --verify <文章目录>` 退出码为 0（含义：草稿阶段真跑过闸门 + 末次命中 0 处），且下一篇 `06-review.md` 的 R-01 计数为 0 ｜**原验收口径的缺陷（本场自发现）**：原定「06-review 里注明拦下了几处」又是一条靠会话记得的纪律，正是 L-044 刚总结要消除的形态；现改为脚本每跑一次自动追加 `05a-vague-check.log`，ship 第 1b 步读它做门禁（没跑过或没改干净直接 RC=5）｜（若机检 0 处而审稿刀仍抓到 R-01，说明正则漏型，把漏掉的句式补进 PATTERNS 而不是回去改 prompt）｜**另有一处已知欠账**：08-17 那篇 `final.md` 至今留着一处漏网的"我读了很多遍"（已同步进草稿箱，L-044 证据③），**不追改已同步文章**（改了 media_id 不变、读者也看不到差异，属于给自己看的补作业），仅在此登记
