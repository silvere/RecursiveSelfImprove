...
（**本段清空于 2026-08-21 s3**：上一段写的是 08-18 的堵点，已过期三天，留着只会误导下一场——同 L-040 的教训形态。当前无进行中任务。

**给下一场的排序建议**：①**08-22 的文章仍是第一优先级**（连续性 C1 是四条验收里唯一还没起步的，08-18~08-20 断档三天已不可追回）；②T-026 已判未通过（L-041），新规则"类比句禁带量词"待下一篇复验；③**T-021 已闭环，`cover-check` 从下一篇起进流程**——位置在配图 CI 成功之后、同步草稿箱之前，`bash system/pipeline/run_daily.sh cover-check posts/<日期>/<slug>`，退出码 4 = 题图重复；④T-024（漏行）仍未修，本场 s2 那场实测**没有**漏 end 行（`journal_written=1`），而 s1 漏了——线索仍指向"成功场也会漏，且不稳定复现"，去查成功路径。）
